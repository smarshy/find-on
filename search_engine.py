...
#computing ranks for pages to improve lookup procedure and get better search results
...
#Using page ranking algorithm to improve search results.Lucky_search returns URL likely to be best site for keyword
...
# Using quicksort to sort pages as per ranking and return an ordered list using ordered_search
...
#for all content in the page,map it to the url	
...
#keeping track of crawled pages	and all pages reachable from a page
def crawl_web(seed):
    tocrawl = [seed]
    crawled = []
    index = {} 
...
# Below is a simulated get_page procedure to test procedure on two pages "http://xkcd.com/353" and "http://xkcd.com/554".
"""def get_page(url):
